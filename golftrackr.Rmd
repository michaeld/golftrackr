...
```{r, echo=FALSE}
ggplot(gto, aes(factor(1), fill = as.ordered(plus_minus_par))) + geom_bar() + facet_wrap(~hole) + coord_polar(theta="y") + labs(fill="score minus par") + ggtitle('Score against Par per Hole')
```

#### Distribution of Scores
```{r, echo=FALSE}
ggplot(gto, aes(fir, as.factor(plus_minus_par))) + geom_point(position=position_jitter(w=0.1, h=0.1)) + ggtitle("Scores when FIR")

ggplot(gto, aes(gir, as.factor(plus_minus_par))) + geom_point(position=position_jitter(w=0.1, h=0.1)) + ggtitle('Scores when GIR')
```
